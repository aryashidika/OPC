export default {
	...
	onUnassign: function () {
		...
		selectedRows.forEach(function (row) {
			const dbRow = parts.find(function (p) { return p.part_id === row.part_id; });
			const dbPackageId = dbRow ? dbRow.cos_package_id : null;
			const dbCosType   = dbRow ? dbRow.cos_type : null;

			const hasNothingToClear =
						(dbPackageId === null || dbPackageId === undefined) &&
						(dbCosType   === null || dbCosType   === undefined);
	...
	onSaveAllocation: async function () {
		...
		try {
			if (pkg.isBefore) {
				...
				await updatePartsForBeforeCosType.run({ sessionId: COSJS.getSessionId(), packageId: pkg.id, cosType });
	...
	onBeforeUnassign: function () {
		...
		selectedRows.forEach(function (row) {
			const dbRow = parts.find(function (p) { return p.part_id === row.part_id; });
			const dbPackageId = dbRow ? dbRow.cos_before_package_id : null;
			const dbCosType   = dbRow ? dbRow.raw_cos_type : null;
