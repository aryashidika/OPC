export default {
	...
	onAssign: function () {
		...
		// Ambil override lead time dari input — null jika kosong
	...
	onClearAssign: function () {
		...
		const parts = getPartsForDivision.data || [];
		...
		selectedRows.forEach(function (row) {
			...
			if (dbValue === null || dbValue === "" || dbValue === undefined) {
				// Di DB sudah null — tidak perlu pending, hapus key saja
				delete BlackboxJS._pendingChanges[row.part_id];
			} else {
				// Di DB ada nilai — perlu pending null untuk overwrite
	...
	getTableData: function () {
		const parts = (getPartsForDivision.data || []).slice().sort(function(a, b) {
		...
		const catalog = getCatalogBlackbox.data || [];
		...
		return parts.map(function (p) {
			...
			if (!hasPending) {
				...
				// Hitung effective: override jika ada, else default catalog
	...
	onSubmit: async function () {
		...
		const openConcerns = getOpenConcernsForBlackbox.data || [];
	...
	async onCopyDivisionConfig() {
		...
		const src = (getDivisionStatus.data || []).find(s => s.division === 'BLACKBOX');
		...
		for (const targetSessionId of targets) {
			...
			try {
				...
				await copyBlackboxProcess.run({ oldSessionId: sourceSessionId, newSessionId: targetSessionId });
				await submitDivision.run({ sessionId: targetSessionId, division: 'BLACKBOX', state: 'IN_WORK', submittedBy: appsmith.user.email });
	...
	async onCloseAllConcernsBlackbox() {
		...
		const concerns = getOpenConcernsForBlackbox.data || [];
		...
		try {
			await Promise.all(concerns.map((c) =>
																		 closeConcernBlackbox.run({ concernId: c.id, closedBy: appsmith.user.email })
																		));
			await getOpenConcernsForBlackbox.run();
